        ...
        formSelects.data('region_select_dtx', "server", {
            ...
            success: function (id, url, searchVal, result) {
            	if (result)
            	{
                    ...
                    layui.formSelects.value('region_select_dtx', regionSelectedBak);    
...
//加载经销商下拉宽
function setDealerSelect_dtx(area,regionIds,businessIds) {
    //赋值大区下拉宽
    formSelects.data('dealer_select_dtx', "server", {
        url: ctx + '/dealerOne/getDealerList?area_ids=' + area + '&is_self=false&level=1' + '&region_ids=' + regionIds + '&business_ids=' + businessIds,
        keyName: 'name',
        keyVal: "id",
        success: function (id, url, searchVal, result) {
        	if (result)
        	{
                ...
                layui.formSelects.value('dealer_select_dtx', dealerSelectedBak);    
...
//赋值商务经理下拉宽
function setBusinessManagerSelect_dtx(area,regionIds,dealerIds) {
    formSelects.data('business_select_dtx', "server", {
        url: ctx + '/dealerOne/getBusinessManagerListForDtx?area_ids=' + area +  + '&region_ids=' + regionIds + '&dealer_ids=' + dealerIds,
        keyName: 'name',
        keyVal: "id",
        beforeSuccess: function (id, url, searchVal, result) {
            result = result.data;
...
//大区下拉事件监听，去取省份的值
formSelects.on('region_select_dtx', function (id, vals, val, isAdd, isDisabled) {
    if (val == undefined) {
        return false;
    }
    var region = "";
    ...
    // 商务经理
    setBusinessManagerSelect_dtx(area_code,region,dealerSelectedBak.join(","));
    
    // 经销商信息
    setDealerSelect_dtx(area_code,regionSelectedBak.join(","),businessUserSelectedBak.join(","));
});

formSelects.on('business_select_dtx', function (id, vals, val, isAdd, isDisabled) {
    if (val == undefined) {
        return false;
    }   
    ...
    setBusinessManagerSelect_dtx(area_code,regionSelectedBak.join(","),dealerSelectedBak.join(","));
});   
